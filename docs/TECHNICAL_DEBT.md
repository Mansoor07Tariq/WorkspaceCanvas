...
| TD-031 | Minor | Frontend | Bookings | Floor map canvas does not colour desk shapes by availability status (available/reserved/bookedByMe/unavailable); colour coding is only shown in the `DeskBookingPage` list view | Users cannot see at-a-glance availability on the canvas; requires cross-slice state sharing between `FloorLayoutPage` and the booking feature | Pass availability items into `FloorMapCanvas` and apply per-object fill colours based on `DeskAvailabilityStatus`; blocked by TD-019 canvas refactor | PR 043 | open |
...
*Last updated: 2026-06-01 — PR 042 booking UI foundation; TD-016 moved to resolved (booking feature slice wired to `/app/bookings`); TD-031 added (canvas availability badge colouring deferred; replaces incorrectly numbered TD-017 which was already used in resolved debt); TD-003, TD-005, TD-006, TD-011 suggested PR updated to PR 043*
