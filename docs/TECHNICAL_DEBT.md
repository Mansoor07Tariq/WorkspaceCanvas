...
|----|----------|----------|------|---------|--------|-----------------|--------------|--------|
| TD-044 | Nit | Frontend | State | Booking hooks (`useDeskBookings`, `useMyBookings`) are not covered by the PR 055 request cache | Booking lists still refetch on every mount (no cross-navigation cache); acceptable since booking data is the most time-sensitive | Add date/status-keyed cache entries + book/cancel invalidation when booking traffic warrants it | PR 056+ | open |
| TD-045 | Nit | Frontend | Multi-org | `FloorLayoutPage`/`OfficeDetailPage` UI role gates use the first active membership, not the selected/per-office membership | A multi-org user whose role differs across orgs may see an incorrect read-only/edit affordance on a non-first org's office; backend enforces the correct per-office permission regardless, so it is a UI-only gap | Resolve the role from the office's org membership (e.g. reuse the selected-org/per-office membership) | PR 056+ | open |
...
*Last updated: 2026-06-02 — PR 055 architecture cleanup. Resolved TD-019 (`useCanvasInteractions` extraction), TD-021 (TTL request cache + invalidation across `useOffices/useFloors/useDesks/useLayoutObjects/useWorkspaceSummary`), and TD-037 (selected-organization provider + switcher + backend `?organization=`/`get_office_for_user`). Full backend (765) and frontend (1141) suites green; lint/format/tsc/build clean; `npm audit` 0 high; no migration drift. New accepted post-MVP debt: TD-044 (booking-hook caching) and TD-045 (per-office role gate on Floor Layout / Office Detail). See `docs/055-architecture-cleanup-hook-caching-multi-org.md`.*
