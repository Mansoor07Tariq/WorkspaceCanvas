...
_TD-046 (bookable/publish UX, PR 057) and TD-048 (PR 061 best-effort rotated containment + fixed rectangular boundary) are the open items. TD-047 (PR 058 `PendingInvitationsPrompt` build-mode type error) was already fixed in PR 059's commit `6e92b7d` (`fontWeight={600}` → `sx={{ fontWeight: 600 }}`) and is verified resolved here — see the Resolved table. PR 059 (renderer registry widening) is a behaviour-preserving refactor. See `docs/059-renderer-registry-widening.md`._
