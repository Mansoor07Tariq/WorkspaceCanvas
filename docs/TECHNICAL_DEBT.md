...
*Last updated: 2026-06-02 — PR 054 dashboard accuracy and booking/canvas test-debt cleanup. Resolved TD-032 (canvas node style selector + tests), TD-033 (booking floor-selection integration test), TD-034 (availability legend layout), and TD-035 (org-wide dashboard summary endpoint `GET /api/offices/summary/` + frontend integration). Remaining open items (TD-019, TD-021, TD-037) are deferred to PR 055 (FloorLayoutPage refactor, hook caching, multi-org switcher). See `docs/054-dashboard-accuracy-booking-canvas-cleanup.md`.*
