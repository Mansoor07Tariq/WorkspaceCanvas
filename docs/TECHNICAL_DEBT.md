...
| TD-042 | Nit | Product-limitation | Frontend / Nav | Events route (`/app/events`) still registered in `AppRouter` but leads to `ComingSoonPage`; removed from sidebar in PR 049 | URL is reachable directly but no longer surfaced in nav; minor dead-weight in the router until Events is implemented or permanently deferred | Remove route and `ComingSoonPage` when Events is confirmed deferred, or implement Events | PR 050+ | open |
