...
| TD-016 | Minor | Frontend | Bookings | Entire bookings feature slice (`useDeskBookings`, `createDeskBooking`, `cancelDeskBooking`, `bookingApi`) has zero production call-sites | Dead code shipped in the bundle; the `/app/bookings` route renders only `ComingSoonPage` | Wire the booking UI or clearly mark the feature as in-progress with a `// TODO:` comment; remove exported symbols that are not yet imported anywhere | PR 044 | open |
...
*Last updated: 2026-05-31 — PR 041 final cleanup; TD-018, TD-022, TD-030 moved to resolved; TD-012, TD-013, TD-014 notes strengthened to reflect full field-level assertions; 8 untracked fixes added to resolved table; TD-011 description clarified to distinguish view-path cancel from signal-based cascade*
