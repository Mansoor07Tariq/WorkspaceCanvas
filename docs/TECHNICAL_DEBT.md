...
| TD-047 | Important | Frontend | Invitations | `frontend/src/features/invitations/components/PendingInvitationsPrompt.tsx:163` fails the build-mode type-check (`tsc -b` → TS2769): a `<Typography>` is used with `noWrap`/`fontWeight` but resolves to the `component`-required overload. `npm run build` (which runs `tsc -b`) fails; `tsc --noEmit` does not surface it. **Pre-existing from PR 058 — not introduced by PR 059** (verified by stashing PR 059 changes and reproducing on the base). | `npm run build` is red on the branch stack; CI build gate blocked until fixed | Add the missing `component` prop (e.g. `component="span"`) or restructure the `Typography` usage so TS picks the default overload, then re-run `npm run build` | PR 060 / hotfix | open |

_TD-046 (bookable/publish UX, PR 057) and TD-047 (PR 058 `PendingInvitationsPrompt` build-mode type error, found during PR 059 checks) are the open items. PR 059 (renderer registry widening) introduced no new debt and resolved none — it is a behaviour-preserving refactor. See `docs/059-renderer-registry-widening.md`._
