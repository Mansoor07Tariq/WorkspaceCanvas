...
class EnhanceRun(models.Model):
    """A record of an "EnhanceRun" layout-enhancement operation against a floor.

    Captures the apply/undo/retry of a batch of geometry patches generated by
    the enhancement planner, including per-operation outcomes for audit and
    idempotency. ``plan_id`` is client-generated for an apply (and enforced
    unique per floor for idempotency); server-generated (uuid4) for undo/retry.
    """
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["floor", "plan_id"],
                name="uniq_floor_plan_id",
            ),
        ]
        indexes = [
            models.Index(
    ...
    def __str__(self) -> str:
        return f"EnhanceRun {self.id} ({self.kind}/{self.status})"


class EnhanceRunOperation(models.Model):
    """A single per-object outcome within an EnhanceRun.

    ``object_id`` is a plain int (not a FK) because the referenced
    FloorLayoutObject may be deleted after the run is recorded.
    """
    ...
    after_geometry = models.JSONField(default=dict, blank=True)
