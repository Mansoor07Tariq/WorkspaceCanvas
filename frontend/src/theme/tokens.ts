...
/**
 * Occupant identity tile inset (PR 080 B3) — the unitless fraction of the desk's top-surface
 * rect left as a wood margin around the tile, applied uniformly by `computeTileBox`. Smaller =
 * fuller fill. Retune here in one line (sensible range 0.0–0.05) while viewing a real floor at
 * map scale; per-asset positioning is `desktopRect` in `manifest.overrides.json`, not this.
 */
